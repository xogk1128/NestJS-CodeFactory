...
export class CommonService {
  ...
  private composeFindOptions<T extends BaseModel>(
     ...
     * 나중에 where__likeCount__more_than 이나 where__title__like 등 추가 필터를 넣고 싶어졌을 때
     * 모든 where 필터들을 자동으로 파싱 할 수 있을만한 기능을 제작해야 한다.
     *
     * 1) where로 시작한다면 필터 로직을 적용한다.
     * 2) order로 시작한다면 정렬 로직을 적용한다.
     * 3) 필터 로직을 적용한다면 '__' 기준으로 split 했을 때 3개의 갑으로 나뉘는지
     *      2개의 값으로 나뉘는지 확인한다.
  ...
  private parseWhereFilter<T extends BaseModel>(
    ...
    if (split.length !== 2 && split.length !== 3) {
      throw new BadRequestException(
        `where 필터는 '__'로 split 했을 때 길이가 2 또는 3이어야 합니다. - 문제되는 key 값 : ${key}`,
      );
    }
    ...
    if (split.length === 2) {
      ...
      /**
       * 길이가 3일 경우에는 Typeorm 유틸리티 적요이 필요한 경우다.
       *
      ...
      //   if (operator === 'between') {
      //     options[filed] = FILTER_MAPPER[operator](values[0], values[1]);
      //   } else {
  ...
  //     if (split.length !== 2) {
  //       throw new BadRequestException(
  //         `order 필터는 '__'로 split 했을 . 때길이가 2여야 합니다. - 문제되는 키 값 : ${key}`,
  //       );
  //     }
