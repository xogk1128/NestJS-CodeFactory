...
export class AuthService {
   ...
   *  튿정 사용자의 토큰이 없다면 다른 사용자의 데이터를 접근 못한다.
   * 5) 모든 토큰은 만료 기간이 있다. 만료기간이 지나면 새로 토큰응 발급받아야 한다.
  ...
  verifyToken(token: string) {
    try {
      return this.jwtService.verify(token, {
        secret: this.configSevice.get<string>(ENV_JWT_SECRET_KEY),
      });
    } catch (e) {
  ...
  async registerWithEmail(user: RegisterUserDto) {
    const hash = await bcrypt.hash(
      user.password,
      parseInt(this.configSevice.get<string>(ENV_HASH_ROUNDS_KEY)),
    );
